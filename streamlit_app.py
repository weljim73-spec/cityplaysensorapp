...
def load_data_from_google_sheets():
    ...
    try:
        ...
        if 'date' in df.columns:
            ...
            # Try to localize to Central Time, but don't fail if there are issues
            try:
                central_tz = pytz.timezone('America/Chicago')
                # Only localize if column has valid dates and is timezone-naive
                if len(df) > 0 and df['date'].notna().any():
                    # Check if already has timezone info
                    if hasattr(df['date'].iloc[0], 'tzinfo'):
                        first_valid = df['date'].dropna().iloc[0] if len(df['date'].dropna()) > 0 else None
                        if first_valid is not None and first_valid.tzinfo is None:
                            df['date'] = df['date'].dt.tz_localize(central_tz, ambiguous='NaT', nonexistent='NaT')
            except:
                # If timezone localization fails, continue without it
                pass
